...
abstract class Builder{
    protected ArrayList<Segment> plansza=new ArrayList<Segment>();

    public abstract void buildSegmentBlock(int x, int y);
    public abstract void buildSegmentBlockV(int x, int y);
    public abstract void buildSegment(int x, int y);
    public abstract void buildSegmentAnim(int x, int y);
    public abstract ArrayList<Segment> getPlansza();
}

class Builder1 extends Builder{
    @Override
    public void buildSegmentBlock(int x, int y){
        Segment s=new SegmentBlock(x, y, "block1.png");
        plansza.add(s);
    }
    @Override
    public void buildSegmentBlockV(int x, int y){
        Segment s=new SegmentBlockV(x, y, "block2.png");
        plansza.add(s);
    }
    @Override
    public void buildSegment(int x, int y){
        Segment s=new Segment(x, y, "block3.png");
        plansza.add(s);
    }
    @Override
    public void buildSegmentAnim(int x, int y){
        Segment s=new SegmentAnim(x, y, "bonus.png", new int[] {0, 0, 0, 1, 1, 1, 2, 2, 3, 3, 2, 2, 1, 1, 1, 0, 0});
        plansza.add(s);
    }
    @Override
    public ArrayList<Segment> getPlansza(){return plansza;}
}

class Builder2 extends Builder1{
    @Override
    public void buildSegment(int x, int y) {
                    ...
                    switch (znak) {
                        ...
                        case 'A':
                            for (int i=0;i<liczba;++i) {
                                b.buildSegmentBlock(x, y);
                                x+=TILESIZE;
                            }
                            break;
                        case 'B':
                            for (int i=0;i<liczba;++i) {
                                b.buildSegmentBlockV(x, y);
                                x+=TILESIZE;
                            }
                            break;
                        case 'C':
